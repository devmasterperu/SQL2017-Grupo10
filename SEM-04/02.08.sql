--4.8
SET IDENTITY_INSERT dbo.UnidadUso ON --HABILITAR LA COLUMNA IDENTITY
...
SET IDENTITY_INSERT dbo.UnidadUso OFF --DESHABILITAR COLUMNA IDENTITY

select count(1) from UnidadUso --998
select * from UnidadUso order by idunidaduso

select * from UnidadUso where idficha in (99998,99999)


--4.8.b

create table unidad_uso_origen
(
id int
)

insert into unidad_uso_origen values
(1),
(2),
(3),
(4),
...
select uo.id,uf.idunidaduso from unidad_uso_origen uo
left join UnidadUso uf on uo.id=uf.idunidaduso
where uf.idunidaduso is null